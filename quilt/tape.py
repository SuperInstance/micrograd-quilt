...
class Tape:
    ...
    # -- integrity -----------------------------------------------------------
    def verify(self):
        """Recompute the chain. Returns (ok, bad_row_index). Internal linkage
        of survivors is verified even after FORGET GC (prefix drops keep the
        suffix chain intact)."""
    ...
    def gc(self, before):
        ...
        graph is rebuilt). A FORGET row re-anchors the chain: it commits
        the drop count and the old tip hash, then survivors are re-chained
        after it, so verify() stays continuous. Tamper evidence for
        SURVIVORS is retained; evidence for the dropped prefix is gone BY
        DESIGN of GC — that is what GC is."""
        assert 0 <= before <= len(self.rows), "gc: prefix drop only"
        old_tip = self._hash
        survivors = self.rows[before:]
        self.rows = []
        self._hash = fnv1a_64(self.GENESIS)
        self._emit({"t": "FORGET", "before": before, "tip": old_tip})
        for r in survivors:
            r = {k: v for k, v in r.items() if k not in ("prev", "hash")}
            self._emit(r)
...
def replay(rows, root=None, after_tick=None, upto_tick=None, init_grads=None):
    ...
    upto_tick=k: stop applying at the k-th TICK (snapshot state).
