...
def leaves(rows):
    """BIND rows never referenced as parents AND never a LINK themselves:
    the true inputs. (A sink also has a BIND row and is nobody's parent —
    without the LINK-id exclusion it would be counted as an input.)"""
    used = {i for r in _links(rows) for i in r["p"]}
    link_ids = {r["id"] for r in _links(rows)}
    return [r["id"] for r in rows
            if r["t"] == "BIND" and r["id"] not in used
            and r["id"] not in link_ids]
...
def audit(rows, float_grads, sink_ids=None, mode="stochastic", seed=0,
    ...
    if mode == "exact":
        ...
        # stratification (documented): the sink is ALWAYS promoted — an
        # audit that never looks at the output would be empty on small
        # graphs; plain Bernoulli(1/sqrt(N)) on the remaining nodes.
        sink_set = set(sink_ids)
        sampled = list(sink_ids) + [
            i for i in ids if i not in sink_set and rng.random() < p]
